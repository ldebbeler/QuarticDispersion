...
writeResults::writeResults(const scalingValues& scaling, const selfEnergyScaling& seScale, const seValuesImag& sev):
    m_scaling(scaling), m_seScale(seScale), m_selfEnergy(sev) {}

void writeResults::write1dvector(H5::H5File file, const std::string& datasetname, const std::vector<double>& vector){
...
// this probably does not work for an empty vector: segmentation fault
void writeResults::write2dvector(H5::H5File file, const std::string& datasetname, const std::vector<std::vector<double>>& vector){
...
void writeResults::createGroup(H5::H5File file, const std::string& groupname){
    file.createGroup(groupname.c_str());
}

void writeResults::writeMainResults(H5::H5File file){
...
// group and data for scaling functions
    ...
    H5::Attribute apos = seScale.createAttribute( "APlus", double_type, att_space );
    apos.write( double_type, &m_seScale.m_Aplus);

    H5::Attribute aneg = seScale.createAttribute( "AMinus", double_type, att_space );
    aneg.write( double_type, &m_seScale.m_Aminus);

    write1dvector(file, "/SelfEnergyScaling/krFit", m_seScale.m_krFit);
    write1dvector(file, "/SelfEnergyScaling/radPosFit", m_seScale.m_radPosFit);
    write1dvector(file, "/SelfEnergyScaling/radNegFit", m_seScale.m_radNegFit);

    write1dvector(file, "/SelfEnergyScaling/ktFit", m_seScale.m_ktFit);
    write1dvector(file, "/SelfEnergyScaling/tangPosFit", m_seScale.m_deltaTangPos);
    write1dvector(file, "/SelfEnergyScaling/tangNegFit", m_seScale.m_deltaTangNeg);
