...
int main() {
    ...
    seScale.m_Aplus = SE.integralPosx(0.0);
    seScale.m_Aminus = SE.integralNegx(0.0);
    seScale.m_krtilde = x;
    ...
    std::vector<double> kradFit(41);
    std::vector<double> radPosFit(41);
    std::vector<double> radNegFit(41);
    std::vector<double> ktangFit(41);
    std::vector<double> tangPosFit(41);
    std::vector<double> tangNegFit(41);
    for(std::size_t i=0; i<kradFit.size(); i++){
        double kr{ -0.2 + 0.01*i };
        kradFit[i] = kr;
        radPosFit[i] = SE.integralPosx(kr);
        radNegFit[i] = SE.integralNegx(kr);
        double kt{ 0.01*(i+1) };
        ktangFit[i] = kt;
        tangPosFit[i] = SE.subtractTangPos(kt);
        tangNegFit[i] = SE.subtractTangNeg(kt);
    }

    seScale.m_krFit = kradFit;
    seScale.m_radPosFit = radPosFit;
    seScale.m_radNegFit = radNegFit;
    seScale.m_ktFit = ktangFit;
    seScale.m_deltaTangPos = tangPosFit;
    seScale.m_deltaTangNeg = tangNegFit;
    ...
    writeResults writeFile(scale, seScale, sev);
