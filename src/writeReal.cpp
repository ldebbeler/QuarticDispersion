...
writeh5::writeh5(const scalingValues& scaling, const bubbleValues& bubble, const seValuesReal& sev):
...
// group and data for bubble with real frequencies
    ...
    write1dvector(file, "/SelfEnergy/variables", m_selfEnergy.m_val);
    write1dvector(file, "/SelfEnergy/realSEPos", m_selfEnergy.m_SEPos);
    write1dvector(file, "/SelfEnergy/realSENeg", m_selfEnergy.m_SENeg);

    H5::Attribute irse = selfEnergy.createAttribute( "IR Cutoff", double_type, att_space );
    irse.write( double_type, &IR);

    H5::Attribute uvse = selfEnergy.createAttribute( "UV Cutoff", double_type, att_space );
    uvse.write( double_type, &UV);
}
